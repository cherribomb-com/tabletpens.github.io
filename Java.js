...
function scrollFunction() {
  if (document.body.scrollTop > 50 || document.documentElement.scrollTop > 50) {
    document.querySelector("w-nav").style.backgroundColor = "rgba(51, 51, 51, 0.5)"; /* change background color to 0.5 opacity */
  } else {
    document.querySelector("w-nav").style.backgroundColor = "rgba(51, 51, 51, 0.8)"; /* change background color back to 0.8 opacity */
  }
}
...
window.onscroll = function() {
    ...
    for (var i = 0; i < pricingCards.length; i++) {
        var cardPosition = pricingCards[i].getBoundingClientRect().top;

        if (cardPosition < screenHeight) {
            pricingCards[i].classList.add('show');
          }
    }
  };
...
//Having 2 onscroll functions just made the top one not work,
//Tried combining them, suddenly they both worked. 